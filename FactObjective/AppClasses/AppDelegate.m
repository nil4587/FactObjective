...
/*
#pragma mark - ===========================================
#pragma mark AppNetworkReachability delegate Methods
#pragma mark ===========================================

- (void)reachabilityDidChange:(NSNotification *)notification {
    AppNetworkReachability *reachability = (AppNetworkReachability *)[notification object];
    if ([reachability isReachable]) {
        NSLog(@"Reachable");
    } else {
        if (@available(iOS 9.0, *)) {
            UIAlertController *alertController = [[UIAlertController alloc] init];
            [alertController setTitle:@"Alert !!"];
            [alertController setMessage:@"Internet connection not available. Please check network setting."];
            [alertController addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                [alertController dismissViewControllerAnimated:TRUE completion:nil];
            }]];
            [alertController presentViewController:self.window.rootViewController animated:TRUE completion:nil];
        } else {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Alert !!" message:@"Internet connection not available. Please check network setting." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alertView show];
        }
    }
}
